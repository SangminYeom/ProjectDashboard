...
it('헤더 날짜 라벨이 격자선과 동일한 좌표(0/25/50/75/100%)에 절대 위치로 정렬된다', () => {
  const { container } = render(
    <Gantt items={tasks} onUpdate={() => {}} onRemove={() => {}} today="2026-06-07" />
  )
  const cells = container.querySelectorAll('.gantt-axis-cell')
  expect(cells).toHaveLength(5)
  expect([...cells].map((c) => c.style.left)).toEqual(['0%', '25%', '50%', '75%', '100%'])
})
